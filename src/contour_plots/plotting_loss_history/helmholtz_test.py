...
class Config:
    ...
    BASE_PATH = "./log_files/checkpoints/helmholtz"
    MODEL_PATHS = {
...
def setup_logger() -> Tuple[Logging, str]:
    ...
    try:
        logger = Logging(Config.TEST_CHECKPOINT_PATH)
...
def create_test_grid() -> torch.Tensor:
    ...
    try:
        dom_coords = torch.tensor([[-1.0, -1.0], [1.0, 1.0]], dtype=torch.float32).to(Config.DEVICE)
        t = torch.linspace(dom_coords[0, 0], dom_coords[1, 0], Config.NUM_POINTS).to(Config.DEVICE).unsqueeze(1)
        x = torch.linspace(dom_coords[0, 1], dom_coords[1, 1], Config.NUM_POINTS).to(Config.DEVICE).unsqueeze(1)
        t, x = torch.meshgrid(t.squeeze(), x.squeeze(), indexing='ij')
        return torch.hstack((t.flatten().unsqueeze(1), x.flatten().unsqueeze(1))).to(Config.DEVICE)
    except Exception as e:
...
def load_model(model_path: str, solver_type: str, logger: Logging) -> Tuple[Any, Dict]:
...
def evaluate_model(model: Any, X_star: torch.Tensor, u_exact: np.ndarray, 
                  f_exact: np.ndarray, logger: Logging) -> Tuple[np.ndarray, np.ndarray]:
