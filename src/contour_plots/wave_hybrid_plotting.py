...
# from src.poisson.classical_solver_new import Classical_Solver # model-1
from src.poisson.classical_solver import Classical_Solver #model-2
...
for model_name, (solver, model_path) in MODEL_PATHS.items():
    if solver == "dv":
        ...
        model.preprocessor.load_state_dict(state["preprocessor"])
        # model.hidden.load_state_dict(state["hidden_network"]) ## uncomment this line for model-1 classical
        model.postprocessor.load_state_dict(state["postprocessor"])
