...
class CVNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.squeezing_phi = (
            torch.randn(num_layers, num_qumodes, device=self.device)  * (2 * np.pi)
        )
    ...
    def apply_gaussian_smoothing(self, x):
        ...
        # print(
        #     f"size after F.conv1d {smoothed.size()}"
        # )  # F.conv1d torch.Size([1, 1, 4])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through the quantum neural network with Gaussian smoothing"""
        outputs = []

        for sample in x:
            result = self.circuit(sample)
            # Apply Gaussian smoothing if the output dimension allows

            if result.numel() > self.kernel_size:
                # print("Applying Gaussian Smoothing")
                result = self.apply_gaussian_smoothing(result)
