...
class CVQuantumLayer(nn.Module):
    ...
    def __init__(self, num_qubits: int, num_layers: int, device):
        ...
        self.displacements = nn.Parameter(torch.randn( 2 , self.num_layers, 
                                                      self.num_qubits, 2, 
                                                      requires_grad=True,
                                                      dtype=torch.float32, 
                                                      device=self.device))
        ...
        self.squeezing = nn.Parameter(torch.randn( 2 , self.num_layers, 
                                                  self.num_qubits, 2, 
                                                  requires_grad=True, 
                                                  dtype=torch.float32, 
                                                  device=self.device))
        ...
        self.beamsplitter = nn.Parameter(torch.randn( 2 , self.num_layers, 
                                                     self.num_qubits-1, 2, 
                                                     requires_grad=True, 
                                                     dtype=torch.float32, 
                                                     device=self.device))
        ...
        self.circuit_X = qml.QNode(self.quantum_circuit_X, self.dev_x, interface="torch")
        self.circuit_P = qml.QNode(self.quantum_circuit_P, self.dev_p, interface="torch")
    ...
    def _initialize_weights(self):
        ...
        torch.nn.init.xavier_normal_(self.displacements.view(2,self.num_layers,self.num_qubits, 2))
        
        torch.nn.init.xavier_normal_(self.squeezing.view(2,self.num_layers,self.num_qubits, 2))
        
        torch.nn.init.xavier_normal_(self.beamsplitter.view(2,self.num_layers,self.num_qubits-1, 2))
        
        
    def quantum_circuit_X(self, inputs, displacements, squeezing, beamsplitter, wire_idx):
        ...
        for layer in range(self.num_layers):
            ...
            for wire in range(self.num_qubits - 1):
                ...
                qml.Beamsplitter(
                    torch.sigmoid(beamsplitter[layer, wire, 0]),  # Ensures value between 0 and 1
                    beamsplitter[layer, wire, 1],
                    wires=[wire, wire + 1]
                )
        
        return qml.expval(qml.X(wire_idx))
        # return [qml.expval(qml.QuadX(wire)) for wire in range(self.num_qubits)]

    def quantum_circuit_P(self, inputs, displacements, squeezing, beamsplitter, wire_idx):
        ...
        # return [qml.expval(qml.QuadP(wire)) for wire in range(self.num_qubits)]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for sample in x:
            ...
            x_measurements = []
            for wire in range(self.num_qubits):
                out_x = self.circuit_X(sample, self.displacements[0], self.squeezing[0], 
                                     self.beamsplitter[0], wire)
                x_measurements.append(out_x)
                
            # Measure P quadrature for each wire
            p_measurements = []
            for wire in range(self.num_qubits):
                out_p = self.circuit_P(sample, self.displacements[1], self.squeezing[1], 
                                     self.beamsplitter[1], wire)
                p_measurements.append(out_p)
            
            # Combine measurements
            combined = torch.cat([
                torch.stack(x_measurements),
                torch.stack(p_measurements)
            ])
