...
def u(x):
    ...
    :param x: x = (t, x)
    """
    return x[:, 1:2] * torch.cos(5 * torch.pi * x[:, 0:1]) + (x[:, 0:1] * x[:, 1:2])**3

def u_tt(x):
    return -25 * torch.pi**2 * x[:, 1:2] * torch.cos(5 * torch.pi * x[:, 0:1]) + 6 * x[:,0:1] * x[:,1:2]**3

def u_xx(x):
    return torch.zeros((x.shape[0], 1), device=x.device) + 6 * x[:,1:2] * x[:,0:1]**3
