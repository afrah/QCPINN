...
class Sampler:
    def __init__(self, dim, coords, func, device="cpu"):
        self.dim = dim
        self.coords = torch.tensor(coords, dtype=torch.float32, device=device)  # Convert coords to float32 tensor
        self.func = func
        self.device = device
...
def u(x, a, c):
    ...
    t = x[:,0:1]
    x = x[:,1:2]
    return torch.sin(torch.tensor(np.pi, dtype=torch.float32, device=x.device) * x) * \
           torch.cos(c * torch.tensor(np.pi, dtype=torch.float32, device=x.device) * t) + \
           a * torch.sin(2 * c * torch.tensor(np.pi, dtype=torch.float32, device=x.device) * x) * \
...
def u_t(x, a, c):
    ...
    u_t = -c * pi * torch.sin(pi * x) * torch.sin(c * pi * t) - \
...
def u_tt(x, a, c):
    ...
    x = x[:,1:2]
    pi = torch.tensor(np.pi, dtype=torch.float32, device=x.device)
    u_tt = -(c * pi)**2 * torch.sin(pi * x) * torch.cos(c * pi * t) - \
           a * (4 * c * pi)**2 * torch.sin(2 * c * pi * x) * torch.cos(4 * c * pi * t)
    return u_tt
...
def u_xx(x, a, c):
    ...
    x = x[:,1:2]
    pi = torch.tensor(np.pi, dtype=torch.float32, device=x.device)
    u_xx = -pi**2 * torch.sin(pi * x) * torch.cos(c * pi * t) - \
           a * (2 * c * pi)**2 * torch.sin(2 * c * pi * x) * torch.cos(4 * c * pi * t)
    return u_xx
